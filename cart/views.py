...
# maybe add_membership_to_cart / remove / adjust
    ...
    if item_sz:
        if item_id in list(cart.keys()):
            if item_sz in cart[item_id]['items_by_sz'].keys():
                ...
                messages.success(
                    request, f'Updated size {item_sz.upper()} \
                         {offer.name} quantity to \
                              {cart[item_id]["items_by_sz"][item_sz]}')
            else:
                cart[item_id]['items_by_sz'][item_sz] = qty
                messages.success(
                    request, f'Added size {item_sz.upper()} \
                        {offer.name} to your cart')
        else:
            cart[item_id] = {'items_by_sz': {item_sz: qty}}
            messages.success(
                request, f'Added size {item_sz.upper()} \
...
def adj_cart(request, item_id):
    ...
    if item_sz:
        if qty > 0:
            ...
            messages.success(
                request, f'Updated size {item_sz.upper()} \
                    {offer.name.upper().upper()} quantity to \
                        {cart[item_id]["items_by_sz"][item_sz]}')
        else:
            ...
            if not cart[item_id]['items_by_sz']:
                cart.pop(item_id)
            messages.success(
                request, f'Removed size {item_sz.upper()} \
                    {offer.name.upper()} from your cart')
    else:
        if qty > 0:
            cart[item_id] = qty
            messages.success(
                request, f'Updated {offer.name.upper()} quantity to \
                    {cart[item_id]}')
        else:
            cart.pop(item_id)
            messages.success(request, f'Removed {offer.name.upper()} \
                from your cart')
    ...
    try:
        ...
        if item_sz:
            ...
            if not cart[item_id]['items_by_sz']:
                cart.pop(item_id)
            messages.success(
                request, f'Removed size {item_sz.upper()} {offer.name.upper()} \
                from your cart')
        else:
            cart.pop(item_id)
            messages.success(request, f'Removed {offer.name.upper()} \
